...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        if not possible_actions:
            return 0
        return max(self.q.get((tuple(state), action), 0) for action in possible_actions)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        best_actions = [action for action, q_value in q_values.items() if q_value == max_value]
        best_action = random.choice(best_actions)

        #Exploit in Reinforcement Learning
        if epsilon == False:
            return best_action
        else:
            #Explore and Exploit
            ...
            probabilities = ([self.epsilon/len(actions) for _ in range(len(actions))]
                             + [(1-self.epsilon)/len(best_actions) for _ in range(len(best_actions))])
            chosen_action = random.choices(all_actions, probabilities, k = 1)[0]
